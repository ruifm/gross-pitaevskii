...
int main(void){
  ...
  h=2.*L/((double)N-1.);//variaveis necessarias e se output
  cout << "gamma = " << g << endl;
  ...
  cout << "Progress:" << endl;
  Complex **psi, **psi1, **psi2; // alocacao de memoria
  psi = new Complex*[N];
  psi1 = new Complex*[N];
  psi2 = new Complex*[N];
  for(i=0;i<N;++i){
  ...
  Complex one(1.,0.);
  Complex zero(0.,0.);
  for(i=0;i<N;++i){   // valores iniciais e normalizacao
    for(j=0;j<N;++j){
  ...
  ofstream out("../../data3a");   // escrita inicial no ficheiro
  for(i=1;i<N-1;++i){
    for(j=1;j<N-1;++j){
  ...
  clock_t t=clock();
  double v=cpf; // variavel que define a frequencia do ciclo de tempo
  int pop=v/cpf;   // pop define o multiplo de cpf com que e feito o refresh da barra de progresso
  sum=0;
  for(n=0;n<nmax;n++){
    if(!(n%cpf)){
      ...
      script << "set title \"Gross-Pitaevskii Bose-Einstein Condensate\"" << endl;
    ...
    for(i=1;i<N-1;++i){   //  implementacao do metodo
      for(j=1;j<N-1;++j){
    ...
    sum=0.;
    for(i=1;i<N-1;++i){  //calculo de N para a normalizacao
      for(j=1;j<N-1;++j){
	sum+=h*h*mod(psi[i][j])*mod(psi[i][j]);
      }
    }
		
    for(i=1;i<N-1;++i){  //normalizacao
      for(j=1;j<N-1;++j){
    ...
    if(!(n%(pop*cpf))){  //actualizacao de v e pop
      t=clock();
      if(v<100.)  
	pop=((int)v)/cpf;
      if(pop==0) pop=1;
  ...
  delete [] psi1;
  delete [] psi2;
  cout << "\n" << endl; //manipulacao de strings usados em system
